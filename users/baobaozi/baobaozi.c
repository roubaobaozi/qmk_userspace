...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    // if (IS_KEY(keycode) || keycode == LTN(Q) || keycode == LTS(W) || keycode == LTK(Z) || keycode == LT0(X) || keycode == LTF(SCLN) || keycode == MC(SLSH) || keycode == MG(QUOT) || keycode == MA(MINS) || keycode == MG(SPC)) {
    //     last_kc = keycode;
    // }

    // other macros
    if (keycode == MA(F21) || keycode == MC(F21)) {
        ...
        if (record->tap.count == 1) {
            ...
            } else if (record->event.pressed && (active_mods & MOD_MASK_SHIFT || active_oneshot_mods & MOD_MASK_SHIFT)) {
                // cancel the sticky or one-shot shift
                del_mods(MOD_LSFT);
...
bool get_permissive_hold(uint16_t keycode, keyrecord_t *record) {
    if (keycode == __T_L1___ || keycode == __T_R1___) {
        // case __T_L1___:
        // case __T_R1___:
        return false;
    }
